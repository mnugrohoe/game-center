...
import { useKingsBoard, calcHasConflict, calcTerritory, type KingsBoardState, type KingsBoardActions } from "../hooks/useKingsBoard";
import { useTimer, type UseTimerReturn } from "../hooks/useTimer";

// ─── Context shape ────────────────────────────────────────────────────────────

export interface KingsBoardContextValue extends KingsBoardState, KingsBoardActions {
...
export function useKingsBoardCtx(): KingsBoardContextValue {
  ...
  if (!ctx) throw new Error("useKingsBoardCtx must be used inside <KingsBoardProvider>");
  return ctx;
}

// ─── Provider ─────────────────────────────────────────────────────────────────

export function KingsBoardProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadPuzzle = useCallback((grid: number[][], N: number) => {
  ...
  const { cellStates, autoLocked } = board;
  ...
  const showHint = useCallback((solution: [number, number][]) => {
    for (const [r, c] of solution) {
      if (cellStates[r]?.[c] !== 2) {
        const el = document.querySelector(`[data-board-cell="${r}-${c}"]`) as HTMLElement | null;
        if (el) {
  ...
  const value = useMemo<KingsBoardContextValue>(() => ({
    ...
    showHint,
    loadPuzzle,
  }), [board, grid, N, numKings, hasAnyConflict, territory, timer, showHint, loadPuzzle]);
